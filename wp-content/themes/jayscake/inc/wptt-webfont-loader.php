...
if ( ! class_exists( 'Animal_Pet_Store_WPTT_WebFont_Loader' ) ) {
	...
	class Animal_Pet_Store_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local jays_cake_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $jays_cake_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $jays_cake_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$jays_cake_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$jays_cake_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$jays_cake_font_family = trim( str_replace( array( "'", ';' ), '', $jays_cake_font_family ) );
					$jays_cake_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $jays_cake_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $jays_cake_font_family ] ) ) {
					$result[ $jays_cake_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$jays_cake_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$jays_cake_font_family_url = $this->get_absolute_path( $jays_cake_font_family_url );

					$result[ $jays_cake_font_family ][] = $jays_cake_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $jays_cake_font_family ] = array_flip( array_flip( $result[ $jays_cake_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_web_jays_cake_font_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_web_jays_cake_font_url( $url, $format = 'woff2' ) {
		$font = new Animal_Pet_Store_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
